...
| 97 | The publication target existed, but executor execution flags were not wired through CLI/Helm for post-failback publication. | Explicit-policy CLI/Helm flags invoke the frontend publication runtime for a failback-source target and require terminal `frontend_published` evidence. | Runtime handoff is proven; workload-visible I/O after publication remains the next gate. |
