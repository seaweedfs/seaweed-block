...
| F2 | Blockmaster/API unreachable | Stop or block access to `sw-blockmaster` before status/report capture | `Unknown`, no `Ready=True`, reason `status_endpoint_unreachable` | port-forward/log artifact, kubectl deploy/pod state, report/explain showing inability to prove readiness | cleanup still succeeds without master API | Product projection test landed; live negative scenario still needed | P0 |
