...
| F2 | Blockmaster/API unreachable | Stop or block access to `sw-blockmaster` before status/report capture | `Unknown` or `Blocked`, no `Ready=True`, reason `blockmaster_unreachable` or equivalent stable code | port-forward/log artifact, kubectl deploy/pod state, report/explain showing inability to prove readiness | cleanup still succeeds without master API | New negative scenario | P0 |
