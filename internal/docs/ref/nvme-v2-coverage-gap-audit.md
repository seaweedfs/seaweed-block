...
| Async Event Request | V2 parks AER. | V3 parks one AER, rejects a second slot, and Phase 127 completes the pending AER on ANA change-count advancement when an ANA provider is wired. | ANA Change source/component present. | Add live Linux host AER validation before making a host-notification release claim. |
