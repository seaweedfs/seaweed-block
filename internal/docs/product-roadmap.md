...
- Phase 105 Multi-Host NVMe/TCP Topology Boundary is closed. It blocks
  ...
  remediation.
- Phase 106 NVMe/TCP Cross-Node Non-Loopback Live Attach is in progress. The
  D1/D2 publish-contract gate is closed: external NVMe/TCP is opt-in at
  `blockvolume`, rendered by blockmaster/launcher/Helm, generated by
  `sw-block ops generate-helm-values --protocol nvme`, and does not leak
  iSCSI/CHAP remediation. The remaining Phase 106 proof is the positive live
  path: CSI stage/publish agreement, app writer/reader I/O, and zero residue
  against the routable NVMe/TCP target.
