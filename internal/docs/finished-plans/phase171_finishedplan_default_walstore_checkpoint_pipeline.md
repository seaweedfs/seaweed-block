# Finished Plan: Phase 171 Default WALStore Checkpoint Pipeline

Status: complete at D1. Correctness hardening and instrumentation shipped; the
bounded extent-write candidate was rejected by its pre-implementation
admission gate, so D2-D6 were intentionally skipped.

## Final Outcome

Phase 171 strengthened the shipped `walstore` before attempting optimization:

- checkpoint metadata is written and fsynced before in-memory checkpoint or
  recyclable WAL-tail publication;
- failed checkpoint publication retains WAL recovery and permits a safe retry;
- `Close` fences new mutations, performs its final flush while the file is
  usable, and waits for in-flight storage work;
- direct rebuild BASE and flusher extent writes have deterministic ownership;
- stale, corrupt, or unsupported dirty WAL records fail the cycle closed;
- full pipeline counters separate validation, WAL readback, extent writeback,
  fsync, checkpoint metadata, cycle outcome, and bounded-write opportunity;
- dual-lane recovery now seals both foreground append and autonomous timer
  drain before its barrier, then restores steady context atomically.

Exact Linux QA on `2c54a61` passed 100 race repetitions of the barrier/timer
tests, 1,000 Pillar3 convergence repetitions, all SIGKILL checkpoint windows,
storage/recovery/transport/replication regression, and storage vet.

The same clean Linux run denied D2:
...
Contiguous writeback had useful individual samples, but sequential opportunity
met the required threshold in only one of five runs. The threshold was not
relaxed after seeing the result. No comparison mode, default change, disk
format, or mounted claim was added.

## Archived Original Plan

The remainder of this document preserves the plan as written before the D1
decision. Prospective language below describes the evaluated hypothesis, not
the final shipped result.
...
At phase start, the default flusher snapshotted the complete dirty map and, for
each current entry, read and validated WAL location metadata, read block data,
issued one extent `WriteAt`, fsynced the shared file, persisted the checkpoint,
and advanced the recyclable tail. This work competed with foreground WAL
append on the same file and exposed only flush-count and byte-count evidence
before D1.
...
Status: complete and independently QA-validated.
...
Status: skipped because the D1 admission gate returned
`d2_bounded_extent_candidate_admitted=false`.
...
Status: skipped with the rejected D2 candidate. WAL read materialization is
carried into Phase 172 as a separate measured hypothesis, not appended to a
failed coalescing candidate.
...
Status: candidate-specific work skipped. D1 nevertheless ran the unchanged
recovery, retention, rebuild, replication, crash, and race regressions.
...
Status: skipped because D2 was not admitted.
...
Status: skipped because D5 had no admitted candidate.
