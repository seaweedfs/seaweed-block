# Finished Plan: Phase 172 WAL Materialization Pipeline

Status: complete with an honest performance rejection. The candidate was
removed and its independent correctness fixes were retained.

## Final Outcome

Phase 172 proved that reducing WAL materialization reads is mechanically
effective but is not the dominant shipped-backend throughput lever:

```text
ordinary reads/validated record: 2.000 -> 1.000
multi-block reads/validated record: 2.000 -> 0.06250
ordinary writers=4 throughput:     78.74 -> 84.09 MiB/s (1.068x)
required writers=4 gain:           1.150x
candidate writers=4 range:         2.031x (required <= 1.500x)
```

D5 therefore closed as `PASS REJECT`; D6 was not run. D7 removed every
single-read/shared-record selector, cache branch, dirty geometry field,
measurement-only counter, candidate test, and obsolete gate. Exact Linux QA on
`811bab2` passed the retained correctness suite 20 times, the same suite under
CGO race 10 times, full storage/recovery/replication/component regressions, and
vet.

The shipped default path remains one header read plus one full-record read.
The legacy range-trim fix and D4 recovery hardening for partial multi-block
suffix replay, malformed batch integrity, physical byte-bound restoration, and
legacy wrapped retained windows remain.
...
Implementation status: complete. Exact-commit Linux D7 passed on `811bab2`;
all candidate absence assertions and retained correctness gates were green.
