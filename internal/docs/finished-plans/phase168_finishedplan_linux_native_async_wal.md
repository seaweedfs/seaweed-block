# Finished Plan: Phase 168 Linux Native Async WAL Execution Milestone

Status: complete as a rejected execution candidate. D1-D4 proved the native
implementation and its correctness boundaries. D5 denied performance
admission after one bounded profile-justified buffer reuse experiment. The
candidate was removed at `954231f`; D6 mounted/RF3 admission was intentionally
skipped.

## Final Outcome

The final controlled m02 gate used one-second samples, rotated implementation
order, five repetitions, per-sample counter validation, 4 KiB Write and
16-block WriteBatch shapes, complete completion-wait syscall tracing, and
...
The owner-confined per-lane buffer reuse removed repeated encode-buffer growth
and stabilized the comparison, but native execution still had no throughput,
scaling, p99, batch, or syscall-count advantage. Further pooling, registered
buffers, or ring tuning would have started a speculative optimization loop
without evidence of architectural benefit.

The rejected code and executable gates remain available in Git history through
`6cf852c`. The surviving product state is deliberate:

- `walstore` remains the default;
- Phase 167 `parallel-walstore` remains opt-in and positioned-I/O only;
- no io_uring selector, fallback, dead executor package, or unusable gate
  remains in the tree;
- the next experiment must reduce persistence rounds through the WAL format
  and commit pipeline rather than replacing `pwrite64` with a more expensive
  one-request ring path.
...
Status: complete at `6cf852c`; performance admission was denied. The revised
exact gate compared native, positioned parallel WAL, and legacy WAL in one m02
session with time-driven rotated samples, median/range, latency, allocation,
native counters, complete wait-path strace, and CPU/allocation profiles.
...
Status: skipped by the D5 stop rule. A rejected local execution path is not
packaged into product/CSI images and cannot consume mounted or RF3 lab time.
