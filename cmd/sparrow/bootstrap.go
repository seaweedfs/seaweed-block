...
//
// Startup logs:
//   "sparrow: durable authority lock acquired"
//   "sparrow: durable authority reload: N records"
//   "sparrow: durable authority reload skip: %v" (one per skip)
//
...
func Bootstrap(storeDir string, directive authority.Directive) (*DurableAuthorityBootstrap, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sparrow: durable authority lock: %w", err)
	}
	log.Printf("sparrow: durable authority lock acquired (store=%q)", storeDir)

	store, err := authority.NewFileAuthorityStore(storeDir)
	if err != nil {
		_ = lock.Release()
		return nil, fmt.Errorf("sparrow: durable authority store: %w", err)
	}
...
func bootstrapFromStoreLocked(storeDir string, lock *authority.StoreLock, store authority.AuthorityStore, directive authority.Directive) (*DurableAuthorityBootstrap, error) {
	...
	if loadErr != nil {
		_ = store.Close()
		_ = lock.Release()
		return nil, fmt.Errorf("sparrow: durable authority index check: %w", loadErr)
	}
	...
	log.Printf("sparrow: durable authority reload: %d records", len(records))
	for _, e := range skips {
