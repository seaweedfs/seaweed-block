...
func TestG15d_K8sRunner_AppliesLauncherGeneratedManifest(t *testing.T) {
	...
	for _, want := range []string{
		...
		"SW_BLOCK_DYNAMIC_REPLICATION_FACTOR",
		"SW_BLOCK_SAME_NODE_LOGICAL_REPLICAS",
		"csi.storage.k8s.io/node-stage-secret-name",
		`sw-block.seaweedfs.com/protocol: \"nvme\"`,
		`protocol: \"nvme\"`,
		"frontend_protocol=$FRONTEND_PROTOCOL",
		"stage2_multipath=$STAGE2_MULTIPATH",
		"dynamic_replication_factor=${DYNAMIC_REPLICATION_FACTOR:-manifest-default}",
		"stage2_multipath:",
		"same_node_logical_replicas=$SAME_NODE_LOGICAL_REPLICAS",
		"--expected-slots-per-volume=${SAME_NODE_LOGICAL_REPLICAS}",
		"failed to override dynamic replicationFactor",
		"failed to inject --stage2-multipath into",
		"failed to pin dynamic pod to same-node NVMe target host",
...
func TestPhase20FirstVolumeScriptPinsUserLoop(t *testing.T) {
	...
	for _, want := range []string{
		...
		"SW_BLOCK_BASIC_APP_PROTOCOL",
		"sw-block.seaweedfs.com/protocol: \\\"nvme\\\"",
		"app_protocol=$BASIC_APP_PROTOCOL",
		"inject_node_stage_secret_into_storageclass",
		"csi.storage.k8s.io/node-stage-secret-name",
		"node_stage_secret=",
		"SW_BLOCK_CLI",
