...
ROOT="${1:-$(pwd)}"
ARTIFACT_DIR="${SW_BLOCK_ARTIFACT_DIR:-${ROOT}/results/phase137-reduce-wal-record-encode-copy-gate}"
SUMMARY="${ARTIFACT_DIR}/phase137-reduce-wal-record-encode-copy-summary.txt"
PHASE126_DIR="${ARTIFACT_DIR}/phase126-profile"
SEQ_MIB="${SW_BLOCK_PHASE137_SEQ_MIB:-512}"
PHASE136_WAL_ENCODE_MS="${SW_BLOCK_PHASE136_WAL_ENCODE_MS:-753}"
PHASE136_WAL_COPY_MS="${SW_BLOCK_PHASE136_WAL_COPY_MS:-593}"
...
recommendation_for_internal_cost() {
  case "$1" in
    wal_copy)
      printf '%s' "phase138_split_remaining_payload_copy"
      ;;
    wal_encode)
      printf '%s' "phase138_wal_encode_allocation_profile"
      ;;
    wal_checksum)
      printf '%s' "phase138_checksum_strategy_profile"
      ;;
    wal_append)
      printf '%s' "phase138_wal_writeat_shape_profile"
      ;;
    dirty_map)
      printf '%s' "phase138_dirty_map_update_profile"
      ;;
    *)
      printf '%s' "phase138_deeper_backend_trace"
      ;;
  esac
}

write_summary "phase137_reduce_wal_record_encode_copy_status=running"
...
write_summary "phase136_wal_encode_duration_ms=${PHASE136_WAL_ENCODE_MS}"
write_summary "phase136_wal_copy_duration_ms=${PHASE136_WAL_COPY_MS}"
write_summary "phase136_wal_encode_copy_duration_ms=$((PHASE136_WAL_ENCODE_MS + PHASE136_WAL_COPY_MS))"

cd "${ROOT}"
go test ./core/storage ./core/frontend/durable \
  >"${ARTIFACT_DIR}/go-test-storage-durable.log" \
  2>&1
write_summary "unit_wal_encode_copy_regression_passed=true"
write_summary "preencode_data_copy_removed=true"
write_summary "batch_append_encodes_direct_to_pending=true"
...
WAL_ENCODE_COPY_MS=$((WAL_ENCODE_MS + WAL_COPY_MS))
PHASE136_WAL_ENCODE_COPY_MS=$((PHASE136_WAL_ENCODE_MS + PHASE136_WAL_COPY_MS))
assert_int_lt "${WAL_ENCODE_COPY_MS}" "${PHASE136_WAL_ENCODE_COPY_MS}" "wal_encode_copy_duration_ms"

POST_PHASE137_BOTTLENECK="$(classify_backend_internal_cost "${WAL_COPY_MS}" "${WAL_ENCODE_MS}" "${WAL_CHECKSUM_MS}" "${WAL_APPEND_MS}" "${DIRTY_MAP_UPDATE_MS}")"
NEXT_RECOMMENDATION="$(recommendation_for_internal_cost "${POST_PHASE137_BOTTLENECK}")"
...
write_summary "wal_encode_copy_duration_ms=${WAL_ENCODE_COPY_MS}"
write_summary "wal_encode_copy_reduced_vs_phase136=true"
write_summary "backend_storage_batching_effective=true"
write_summary "post_phase137_bottleneck=${POST_PHASE137_BOTTLENECK}"
write_summary "next_recommendation=${NEXT_RECOMMENDATION}"
write_summary "cleanup_status=${CLEANUP_STATUS}"
write_summary "phase137_reduce_wal_record_encode_copy_status=ok"
