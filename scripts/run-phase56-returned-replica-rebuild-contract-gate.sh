...
assert_file_contains "summary_rebuild_action_disabled" "${SUMMARY_TXT}" '^managed_volume_action=authority\.rebuild_returned_replica mode=dry_run side_effect=authority_mutating executor=authority_recovery_executor decision=rejected reason=policy_disabled mutation_allowed=false'
