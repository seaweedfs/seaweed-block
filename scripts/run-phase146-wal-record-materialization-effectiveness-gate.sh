...
ROOT="${1:-$(pwd)}"
ARTIFACT_DIR="${SW_BLOCK_ARTIFACT_DIR:-${ROOT}/results/phase146-wal-record-materialization-effectiveness-gate}"
SUMMARY="${ARTIFACT_DIR}/phase146-wal-record-materialization-effectiveness-summary.txt"
PHASE126_DIR="${ARTIFACT_DIR}/phase126-profile"
CANDIDATE_MAX_H2C_BYTES="${SW_BLOCK_PHASE146_CANDIDATE_MAX_H2C_BYTES:-65536}"
SEQ_MIB="${SW_BLOCK_PHASE146_SEQ_MIB:-512}"
MATERIALIZATION_CHANGE="${SW_BLOCK_PHASE146_MATERIALIZATION_CHANGE:-writebatch_value_entries}"
BASELINE_ENCODE_MS="${SW_BLOCK_PHASE146_BASELINE_ENCODE_MS:-297}"
BASELINE_APPEND_MS="${SW_BLOCK_PHASE146_BASELINE_APPEND_MS:-295}"
VISIBLE_THRESHOLD_PCT="${SW_BLOCK_PHASE146_VISIBLE_THRESHOLD_PCT:-5}"
...
classify_effectiveness() {
  python3 - "$BASELINE_ENCODE_MS" "$BASELINE_APPEND_MS" "$1" "$2" "$VISIBLE_THRESHOLD_PCT" <<'PY'
import sys

baseline_encode = int(sys.argv[1])
baseline_append = int(sys.argv[2])
current_encode = int(sys.argv[3])
current_append = int(sys.argv[4])
threshold_pct = float(sys.argv[5])

baseline_pair = baseline_encode + baseline_append
current_pair = current_encode + current_append
improvement_pct = ((baseline_pair - current_pair) * 100.0) / baseline_pair
...
if improvement_pct >= threshold_pct:
    ...
    decision = "needs_deeper_design"
    next_recommendation = "phase147_wal_multiblock_record_design_gate"
else:
    effectiveness = "not_visible"
    decision = "needs_deeper_design"
    next_recommendation = "phase147_wal_multiblock_record_design_gate"

print(f"phase146_baseline_pair_ms={baseline_pair}")
print(f"phase146_current_pair_ms={current_pair}")
print(f"phase146_pair_improvement_pct={improvement_pct:.2f}")
print(f"phase146_effectiveness={effectiveness}")
print(f"phase146_decision={decision}")
print(f"next_recommendation={next_recommendation}")
PY
}

write_summary "phase146_wal_record_materialization_effectiveness_status=running"
...
write_summary "phase146_baseline_encode_ms=${BASELINE_ENCODE_MS}"
write_summary "phase146_baseline_append_ms=${BASELINE_APPEND_MS}"
write_summary "phase146_visible_threshold_pct=${VISIBLE_THRESHOLD_PCT}"
...
while IFS= read -r line; do
  write_summary "${line}"
done < <(classify_effectiveness "${WAL_ENCODE_MS}" "${WAL_APPEND_MS}")
write_summary "cleanup_status=${CLEANUP_STATUS}"
write_summary "phase146_wal_record_materialization_effectiveness_status=ok"
