...
WORK_DIR="${SW_BLOCK_ISCSI_MPATH_WORK_DIR:-/tmp/sw-block-iscsi-mpath}"
ARTIFACT_DIR="${SW_BLOCK_ARTIFACT_DIR:-${WORK_DIR}/runs/${RUN_ID}}"
IQN="${SW_BLOCK_ISCSI_IQN:-iqn.2026-05.io.seaweedfs:mpath-v1}"
PORT1="${SW_BLOCK_ISCSI_PORT1:-3270}"
PORT2="${SW_BLOCK_ISCSI_PORT2:-3271}"
MASTER_ADDR="${SW_BLOCK_MASTER_ADDR:-127.0.0.1:19533}"
R1_DATA_ADDR="${SW_BLOCK_R1_DATA_ADDR:-127.0.0.1:19501}"
R1_CTRL_ADDR="${SW_BLOCK_R1_CTRL_ADDR:-127.0.0.1:19502}"
R1_STATUS_ADDR="${SW_BLOCK_R1_STATUS_ADDR:-127.0.0.1:19503}"
R2_DATA_ADDR="${SW_BLOCK_R2_DATA_ADDR:-127.0.0.1:19511}"
R2_CTRL_ADDR="${SW_BLOCK_R2_CTRL_ADDR:-127.0.0.1:19512}"
R2_STATUS_ADDR="${SW_BLOCK_R2_STATUS_ADDR:-127.0.0.1:19513}"
...
sudo iscsiadm -m session -P 3 >"$ARTIFACT_DIR/iscsi-session-P3.txt" 2>&1 || true
...
lsblk "$real1" "$real2" >"$ARTIFACT_DIR/lsblk.paths.txt" 2>&1 || true

capture_path_evidence() {
  local name="$1"
  local dev="$2"
  sudo sg_inq "$dev" >"$ARTIFACT_DIR/sg_inq.${name}.standard.txt" 2>&1
  sudo sg_inq -p 0x00 "$dev" >"$ARTIFACT_DIR/sg_inq.${name}.vpd00.txt" 2>&1
  sudo sg_inq -p 0x83 "$dev" >"$ARTIFACT_DIR/sg_inq.${name}.vpd83.txt" 2>&1
  sudo sg_rtpg "$dev" >"$ARTIFACT_DIR/sg_rtpg.${name}.txt" 2>&1
}

log "capture ALUA path evidence"
capture_path_evidence r1 "$real1"
capture_path_evidence r2 "$real2"

extract_aas() {
  sed -n 's/.*asymmetric access state[[:space:]]*:[[:space:]]*\(0x[0-9a-fA-F]\+\).*/\1/p' "$1" | head -n1
}

state1="$(extract_aas "$ARTIFACT_DIR/sg_rtpg.r1.txt")"
state2="$(extract_aas "$ARTIFACT_DIR/sg_rtpg.r2.txt")"
log "rtpg_state_r1=${state1:-missing}"
log "rtpg_state_r2=${state2:-missing}"

if [[ -z "$state1" || -z "$state2" ]]; then
  echo "missing REPORT TARGET PORT GROUPS asymmetric access state" >&2
  exit 1
fi
if [[ "$state1" != "0x00" && "$state2" != "0x00" ]]; then
  echo "no active/optimized path reported by RTPG" >&2
  exit 1
fi
if [[ "$state1" != "0x02" && "$state2" != "0x02" ]]; then
  echo "no standby path reported by RTPG" >&2
  exit 1
fi

if ! grep -Eiq "target port group|relative target port|NAA" "$ARTIFACT_DIR/sg_inq.r1.vpd83.txt"; then
  echo "r1 VPD 0x83 missing NAA/target-port identity evidence" >&2
  exit 1
fi
if ! grep -Eiq "target port group|relative target port|NAA" "$ARTIFACT_DIR/sg_inq.r2.vpd83.txt"; then
  echo "r2 VPD 0x83 missing NAA/target-port identity evidence" >&2
  exit 1
fi
...
log "verify standby path rejects raw WRITE"
if sudo dd if=/dev/zero of="$standby_dev" bs=4096 count=1 oflag=direct status=none >"$ARTIFACT_DIR/standby-write.stdout" 2>"$ARTIFACT_DIR/standby-write.stderr"; then
  echo "standby path accepted WRITE unexpectedly" >&2
  exit 1
fi
...
sudo multipath -r >"$ARTIFACT_DIR/multipath.reload.txt" 2>&1 || true
sleep 2
...
' "$ARTIFACT_DIR/multipath.ll.txt" >"$ARTIFACT_DIR/multipath-map-name.txt"; then
  cat "$ARTIFACT_DIR/multipath.ll.txt" >&2
  echo "multipath did not show one map containing both ${base1} and ${base2}" >&2
  exit 1
fi

log "multipath_map=$(cat "$ARTIFACT_DIR/multipath-map-name.txt")"
...
log "PASS: two iSCSI paths report ALUA and multipath groups one logical device"
