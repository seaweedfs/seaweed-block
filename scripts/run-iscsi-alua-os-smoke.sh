...
WORK_DIR="${SW_BLOCK_ISCSI_ALUA_WORK_DIR:-/tmp/sw-block-iscsi-alua}"
ARTIFACT_DIR="${SW_BLOCK_ARTIFACT_DIR:-${WORK_DIR}/runs/${RUN_ID}}"
IQN="${SW_BLOCK_ISCSI_IQN:-iqn.2026-05.io.seaweedfs:alua-v1}"
PORT="${SW_BLOCK_ISCSI_PORT:-3268}"
MOUNT_DIR="${SW_BLOCK_ISCSI_MOUNT_DIR:-${WORK_DIR}/mnt}"
MASTER_ADDR="${SW_BLOCK_MASTER_ADDR:-127.0.0.1:19433}"
DATA_ADDR="${SW_BLOCK_DATA_ADDR:-127.0.0.1:19401}"
CTRL_ADDR="${SW_BLOCK_CTRL_ADDR:-127.0.0.1:19402}"
STATUS_ADDR="${SW_BLOCK_STATUS_ADDR:-127.0.0.1:19403}"
...
if [[ -z "$dev" ]]; then
  ...
  (sudo dmesg | tail -n 120) >"$ARTIFACT_DIR/dmesg.tail.txt" 2>&1 || true
  echo "device not found for IQN ${IQN}" >&2
  exit 1
fi
real_dev="$(readlink -f "$dev")"
log "device=$dev real=$real_dev"
lsblk "$real_dev" >"$ARTIFACT_DIR/lsblk.txt" 2>&1 || true

log "capture ALUA inquiry evidence"
sudo sg_inq "$real_dev" >"$ARTIFACT_DIR/sg_inq.standard.txt" 2>&1
sudo sg_inq -p 0x00 "$real_dev" >"$ARTIFACT_DIR/sg_inq.vpd00.txt" 2>&1
sudo sg_inq -p 0x83 "$real_dev" >"$ARTIFACT_DIR/sg_inq.vpd83.txt" 2>&1
sudo sg_rtpg "$real_dev" >"$ARTIFACT_DIR/sg_rtpg.txt" 2>&1

if ! grep -Eiq "TPGS|target port group|asymmetric" "$ARTIFACT_DIR/sg_inq.standard.txt" "$ARTIFACT_DIR/sg_rtpg.txt"; then
...
log "PASS: iscsiadm login reports ALUA and completes mkfs mount write/read logout"
